...
import { generateBoard, getRandomChallenge } from './utils';

// --- COMPONENTE CARTA ---
const Card = ({ data, viewMode, onClick, isSelected }) => {
  const colors = {
  ...
  return (
    <div onClick={onClick} className={`${baseClass} ${colorClass} ${revealedClass} ${selectionClass}`}>
      {data.word}
...
const DrawingBoard = ({ isOpen, onClose, isCaptain, roomCode, existingImage }) => {
  ...
  useEffect(() => {
    // 1. Siempre actualizamos la imagen visual
  ...
  const saveToFirebase = () => {
    ...
    // No actualizamos localImage aquí para evitar parpadeos, dejamos que el useEffect lo haga
  ...
  const startDraw = (e) => {
    ...
    ctx.beginPath();
    ctx.moveTo(x, y);
    // e.preventDefault(); // Comentado para permitir scroll si es necesario fuera del canvas, pero dentro suele ser mejor prevenir
  };
  ...
  const endDraw = () => {
    if (!isDrawing) return;
    setIsDrawing(false);
    saveToFirebase(); // Guardamos cada trazo
  };
    ...
    <div className="bg-slate-800 text-white p-6 rounded-xl max-w-md w-full shadow-2xl border border-slate-600 overflow-y-auto max-h-[90vh]" onClick={e => e.stopPropagation()}>
      ...
      <div className="space-y-5 text-sm text-gray-300 leading-relaxed">
        ...
        <div className="bg-red-900/30 p-4 rounded-lg border border-red-800/50">
          <h3 className="text-red-400 font-bold text-lg mb-1">⚠️ PROHIBIDO</h3>
...
export default function App() {
  ...
  useEffect(() => {
    ...
    if (roomParam) { setRoomCode(roomParam.toUpperCase()); setView('role_selection'); }
  }, []);
  ...
  const createRoom = () => {
    ...
    set(ref(db, `rooms/${newCode}`), {
      ...
      paused: config.hardMode ? true : false,
      drawing: null
    });
    setView('captain');
  };
  ...
  if (view === 'role_selection') return (<div className="min-h-screen bg-slate-900 flex flex-col items-center justify-center text-white p-4"><h2 className="text-2xl font-bold mb-8">SALA <span className="text-amber-500">{roomCode}</span></h2><div className="grid gap-6 w-full max-w-md"><button onClick={() => handleRoleSelection('table')} className="bg-slate-700 p-6 rounded-2xl border-2 border-slate-500 flex flex-col items-center"><span className="text-4xl">📺</span><span className="font-bold">MODO MESA</span></button><button onClick={() => handleRoleSelection('captain')} className="bg-amber-600 p-6 rounded-2xl border-2 border-amber-400 flex flex-col items-center"><span className="text-4xl">🕵️‍♂️</span><span className="font-bold">MODO CAPITÁN</span></button></div></div>);
